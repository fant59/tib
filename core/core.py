...
class SandBox:
    ...
    def init(self) -> None:
        ...
        self.account = self.sandbox.register_sandbox_account(body)
        self._is_init = True

    def get_market_stocks(self) -> None:
        r = self.sandbox.get_market_stocks()

        for i in r.payload.instruments:
            _i = i.dict()
            _ = _i.pop('type')
            figi = _i.pop('figi')
            obj, _ = \
    ...
    def get_candles(self, deep_year: int = 10,
        ...
        subquery = self.DayCandle.objects.\
            filter(stock=OuterRef('pk')).\
            order_by('-time')
        from_date = Cast(Subquery(subquery.values('time')[:1]),
                         output_field=DateTimeField())
        qs = self.Stock.objects.\
            annotate(from_date=from_date).\
            values('figi', 'from_date', 'id')
        ...
        for stock in qs:
            ...
            while from_date <= today:
                ...
                r = self.sandbox.get_market_candles(
                    stock['figi'],
                    this_from,
                    this_to,
                    CandleResolution.day
                )
                generator = (i for i in r.payload.candles)
                for i in generator:
                    _i = i.dict()
                    time = _i.pop('time')
                    _ = _i.pop('figi')
                    _ = _i.pop('interval')
                    obj, _ = self.DayCandle.objects.\
                        update_or_create(stock_id=stock['id'],
                                         time=time,
                                         defaults=_i)
